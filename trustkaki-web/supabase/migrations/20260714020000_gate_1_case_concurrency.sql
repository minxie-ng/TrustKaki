-- Gate 1: conflict-safe and idempotent shared caregiver case commands.

alter table public.caregiver_actions
  add column if not exists command_id uuid,
  add column if not exists assigned_caregiver_id uuid references public.caregivers(id) on delete set null,
  add column if not exists snoozed_until timestamptz;

update public.caregiver_actions
...
create unique index if not exists caregiver_actions_command_id_idx
  on public.caregiver_actions(command_id);

drop function if exists public.record_caregiver_queue_action(
...
begin
  ...
  v_action_assignment_id := case
    when p_action_type = 'assign' then coalesce(p_assigned_caregiver_id, v_actor_caregiver_id)
    else null
  end;
  ...
  if found then
    if v_existing_action.queue_item_id <> p_queue_item_id
       ...
       or (p_snoozed_until is not null and v_existing_action.snoozed_until is distinct from p_snoozed_until) then
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'caregiver_queue_items'
  ) then
  ...
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'caregiver_actions'
  ) then
